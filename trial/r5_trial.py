...
class Retrieve4Trial(pytry.NengoTrial):
    def params(self):
        ...
        self.param('inhibition strength', inh_strength=1.0)
        self.param('test foils', foils=False)
    ...
    def model(self, p):
        ...
        for i in range(p.M / 6):
            for j in range(3):
                ...
                items.append(pp.v)
                fan.append(1)
                pp = vocab.parse('X%d+Z%d+%g*Q' % (6*i+j, 6*i+j, p.common))
                pp.normalize()
                foils.append(pp.v)
            ...
            for j, v in enumerate(['AB', 'CB', 'AC']):
                ...
                items.append(pp.v)
                fan.append(2)
                pp = vocab.parse('%s%d+D%d+%g*Q' % (v[0], i, i, p.common))
                pp.normalize()
                foils.append(pp.v)
        ...
        with model:
            ...
            nengo.Connection(model.mem.output, model.mem.input,
                             transform=(np.eye(len(items))-1)*p.inh_strength,
                             synapse=p.syn)
        ...
        with model:
            targets = foils if p.foils else items
            self.env = Env(targets)
